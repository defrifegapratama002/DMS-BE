...
// Helper: sama seperti di controller lain — Super Admin & Company Admin bebas,
// Employee hanya boleh mengelola share untuk dokumen yang folder induknya milik dia.
function canManageShare(userRole: string, userId: string, folderOwnerId: string): boolean {
...
export const shareDocument = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!canManageShare(req.user!.role, req.user!.userId, document.folder.owner_id)) {
      res.status(403).json({ message: 'Anda tidak memiliki izin untuk membagikan dokumen ini.' });
      return;
    }

    // Cegah berbagi dokumen ke diri sendiri (tidak ada gunanya, membingungkan di UI)
    if (user_id === req.user!.userId) {
      res.status(400).json({ message: 'Tidak dapat membagikan dokumen ke akun Anda sendiri.' });
      return;
    }
    ...
    // Cek apakah sudah pernah dibagikan sebelumnya — kalau ada, perbarui saja
    // level aksesnya daripada membuat entri duplikat.
    const existingShare = await prisma.documentShare.findFirst({
      where: { document_id: documentId, user_id },
    });
    ...
    res.status(500).json({ message: 'Terjadi kegagalan server saat membagikan dokumen.' });
  }
};

// 2. FUNGSI MELIHAT DAFTAR USER YANG DIBAGIKAN AKSES UNTUK SATU DOKUMEN
export const getDocumentShares = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id: documentId } = req.params as { id: string };
    ...
    if (!canManageShare(req.user!.role, req.user!.userId, document.folder.owner_id)) {
      res.status(403).json({ message: 'Anda tidak memiliki izin untuk melihat daftar akses dokumen ini.' });
      return;
    }
    ...
    res.status(500).json({ message: 'Terjadi kegagalan server saat mengambil daftar akses dokumen.' });
  }
};

// 3. FUNGSI MELIHAT DOKUMEN YANG DIBAGIKAN KEPADA SAYA (dari sudut pandang penerima)
export const getSharedWithMe = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Terjadi kegagalan server saat mengambil dokumen yang dibagikan.' });
  }
};

// 4. FUNGSI MENGUBAH LEVEL AKSES BERBAGI
export const updateShareAccess = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!canManageShare(req.user!.role, req.user!.userId, share.document.folder.owner_id)) {
      res.status(403).json({ message: 'Anda tidak memiliki izin untuk mengubah akses ini.' });
      return;
    }
    ...
    res.status(200).json({ message: 'Level akses berhasil diperbarui.', share: updated });
  } catch (error) {
    console.error('Update Share Access Error:', error);
    res.status(500).json({ message: 'Terjadi kegagalan server saat memperbarui akses berbagi.' });
  }
};
...
export const revokeShare = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!canManageShare(req.user!.role, req.user!.userId, share.document.folder.owner_id)) {
      res.status(403).json({ message: 'Anda tidak memiliki izin untuk mencabut akses ini.' });
      return;
    }
    ...
    res.status(500).json({ message: 'Terjadi kegagalan server saat mencabut akses berbagi.' });
  }
};